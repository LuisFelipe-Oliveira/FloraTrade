...
$sql_code = "SELECT * FROM Usuario WHERE favorito = 'S' ORDER BY IdUsuario";
$sql_query_usuario = $conn->query($sql_code)->fetchAll(PDO::FETCH_ASSOC);
...
<body>
  ...
  <div class="container">
    ...
    <h2>Usuario</h2>
    <p>Listagem de usuarios favoritos.</p>
    ...
    <table class="table table-striped table-bordered table-hover">
      ...
      <tbody>
        <?php
        if (count($sql_query_usuario) > 0) {
          foreach ($sql_query_usuario as $data) { ?>
            <tr>
              <th scope="row" style="text-align:center">
                <?php echo $data['IdUsuario']; ?>
              </th>
              <td scope="row" style="text-align:center"><img class="imgPerfilTabela"
                        src="<?php echo $data['FotoPerfil']; ?>" alt=""></td>
              <td>
                <?php echo $data['Nome']; ?>
              </td>
              ...
              <td scope="row" style="text-align:center">
                <form action="usuario-remove-favorite.php" method="post">
                  <input type="hidden" name="id_Usuario" value="<?php echo $data['IdUsuario']; ?>">
        ...
        if (count($sql_query_fornecedor) > 0) {
          foreach ($sql_query_fornecedor as $data) { ?>
            <tr>
              ...
              <td scope="row" style="text-align:center">
                <?php echo $data['NomeFornecedor']; ?>
              </td>
              <td scope="row" style="text-align:center">
                <?php echo $data['CNPJ']; ?>
              </td>
              <td scope="row" style="text-align:center">
                <?php echo date('d/m/Y', strtotime($data['DataCadastro'])); ?>
              </td>
              <td scope="row" style="text-align:center">
        ...
        if (count($sql_query_venda) > 0) {
          foreach ($sql_query_venda as $data) { ?>
            <tr>
              ...
              <td style="text-align:center">
                <?= selectUsuario($conn, $data['IdUsuario']) ?>
              </td>
              <td style="text-align:center">
                <?= selectCliente($conn, $data['IdCliente']) ?>
              </td>
              <td scope="row" style="text-align:center">
